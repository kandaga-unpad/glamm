defmodule GlammWeb.CreatorController do
  ...
  def index(conn, _params) do
    mst_creator = Master.list_mst_creator()
    render(conn, :index, mst_creator: mst_creator)
  end

  def new(conn, _params) do
    changeset = Master.change_creator(%Creator{})
  ...
  def update(conn, %{"id" => id, "creator" => creator_params}) do
    ...
    case Master.update_creator(creator, creator_params) do
      {:ok, creator} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    creator = Master.get_creator!(id)
    {:ok, _creator} = Master.delete_creator(creator)
